  ...
  const sendOrUpdate = function(embed) {
    if (MESSAGE !== undefined) {
      MESSAGE.edit(embed).then(() => {
        log(LOG_LEVELS.DEBUG, '✅ Frissítés sikeres');
      }).catch((e) => {
      ...
      if (channel !== undefined) {
        channel.messages.fetch(MESSAGE_ID).then((message) => {
          MESSAGE = message;
          message.edit(embed).then(() => {
            log(LOG_LEVELS.SPAM, '✅ Frissítés sikeres');
          }).catch((e) => {
            log(LOG_LEVELS.ERROR, `❌ Sikertelen frissítés\nHiba: ${e}`);
          });
        }).catch(() => {
          channel.send(embed).then((message) => {
            MESSAGE = message;
            log(LOG_LEVELS.INFO,`✅ Állapotüzenet elküldve.\nKérjük, frissítse a konfigurációs fájlt ezzel az üzenetazonosítóval 𓊈${message.id}𓊉`);
          }).catch(console.error);
        })
      } else {
        log(LOG_LEVELS.ERROR, '❌ Frissítési csatorna nincs beállítva');
      }
    }
  };
bot.on('ready', () => {
var checkMe = ['ADMINISTRATOR','CREATE_INSTANT_INVITE','KICK_MEMBERS','BAN_MEMBERS','MANAGE_GUILD','ADD_REACTIONS','VIEW_AUDIT_LOG','PRIORITY_SPEAKER' ,'VIEW_CHANNEL','SEND_MESSAGES','SEND_TTS_MESSAGES','MANAGE_MESSAGES','READ_MESSAGE_HISTORY','MENTION_EVERYONE','USE_EXTERNAL_EMOJIS' ,'VIEW_GUILD_INSIGHTS','CONNECT','SPEAK','MUTE_MEMBERS','DEAFEN_MEMBERS','MOVE_MEMBERS','USE_VAD','CHANGE_NICKNAME','MANAGE_NICKNAMES','MANAGE_ROLES','MANAGE_WEBHOOKS','MANAGE_EMOJIS','STREAM','EMBED_LINKS','ATTACH_FILES','MANAGE_CHANNELS']  
  ...
  console.log(`⚠ FIGYELMEZTETÉS: A 'PERMISSION' változó (${PERMISSION}) hibás, kérjük, ellenőrizze, hogy megtalálja az engedélyek listáját... kilépés....`);
  process.exit(0);             
  }
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor(`${SERVER_NAME} | Szerver állapota`, SERVER_LOGO)
    .setColor(EMBED_COLOR)
    .setThumbnail(SERVER_LOGO)
    .setFooter(TICK_N % 2 === 0 ? `|| ${SERVER_NAME} ||` : `|| Edited by: .notixX™#9755 ||`)
    .setTimestamp(new Date())
    .addField('\n\u200b\nSzerver Név', `\`\`\`${SERVER_NAME}\`\`\``,false)
    if (STATUS !== undefined)
    {
      embed.addField('📬 Szerver értesítés:',`\`\`\`${STATUS}\`\`\`\n\u200b\n`);
  ...
  const offline = function() {
    ...
    .setThumbnail(SERVER_LOGO)
    .addFields(
      { name: "Szerver állapota:",          value: "```❌ Offline```",    inline: true },
      { name: "Várólista:",                value: "```--```",            inline: true },
      { name: "Online Játékos:",         value: "```--```\n\u200b\n",  inline: true },
      { name: "Szerver IP:",   value: "```N/A```",           inline: true },
      { name: "Szerver újraindítási idők:",   value: "```N/A```",           inline: true }
    )
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addFields(
          { name: "Szerver állapota:",            value: "```✅ Online```",                                                                                    inline: true },
          { name: "Várólista:",                  value: `\`\`\`${queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim()}\`\`\``,        inline: true },
          { name: "Online Játékos:",           value: `\`\`\`${players.length}/${MAX_PLAYERS}\`\`\`\n\u200b\n`,                                              inline: true },
          { name: "Szerver IP:",    value: `\`\`\`${SERVER_IP}\`\`\`\n\n`,                                                                        inline: true },
          { name: "Szerver újraindítási idők:",    value: `\`\`\`${RESTART_TIMES}\`\`\``,                                                                        inline: true }
          )
        .setThumbnail(SERVER_LOGO)
        if (players.length > 0) {
    ...
    return new Promise((resolve,reject) => {
      ...
      let id = loop_callbacks.push(() => {
        if (!resolved) {
          ...
          log(LOG_LEVELS.ERROR, 'A loop visszahívása időtúllépés után');
          reject(null);
        }
      })
      setTimeout(() => {
  ...
  bot.on('disconnect',(devent,shard) => {
    ...
    checkLoop().then((running) => {
      log(LOG_LEVELS.INFO, `Loop még fut : ${running}`);
    }).catch(console.error);
  })
  ...
  bot.on('message', async function (msg) {
    ...
    if (msg.content === '+help') {
      ...
      .setDescription('+status <Üzenet> - Figyelmeztető üzenetet ad a szerver állapot beágyazásához\n+állapot törlése - Törli a figyelmeztető üzenetet\n+help - Megjeleníti a bot parancsokat')
      .setTimestamp(new Date());
      ...
      let noPerms =  new Discord.MessageEmbed()
        ...
        .setDescription(`❌ Nem rendelkezik a ${PERMISSION} jogosultsággal, ezért nem tudja futtatni ezt a parancsot!`)
        .setTimestamp(new Date());
    ...
    if (!message.author.bot) {
      if (message.member) {
          ...
          if (message.content.startsWith('+status ')) {
            ...
            .setColor(EMBED_COLOR)
            .setTitle('☑️ Frissített állapotüzenet')
            .setTimestamp(new Date());
            if (status === 'clear') {
              STATUS = undefined;
              ...
              STATUS = status;
              embed.setDescription(`Új üzenet:\n\`\`\`${STATUS}\`\`\``);
            }
            bot.channels.cache.get(LOG_CHANNEL).send(embed);
            return log(LOG_LEVELS.INFO, `🔘 ${message.author.username} frissített állapot`);
          } else {
            let noPerms =  new Discord.MessageEmbed()
              .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag, message.author.displayAvatarURL())
              .setColor(0x2894C2)
        ...
        if (message.channel.id === SUGGESTION_CHANNEL) {
          ...
          message.channel.send(embed).then((message) => {
            ...
            sent.react('👍').then(() => {
              sent.react('👎').then(() => {
                log(LOG_LEVELS.SPAM, 'Kész javaslat üzenet');
              }).catch(console.error);
            }).catch(console.error);
          }).catch(console.error);
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Jelentését sikeresen elküldtük a személyzetnek!')
          .setTimestamp(new Date());
  ...
  bot.login(BOT_TOKEN).then(null).catch(() => {
    log(LOG_LEVELS.ERROR, 'A megadott token érvénytelen. Kérjük, győződjön meg arról, hogy a megfelelőt használja!');
    console.error(e);
    process.exit(1);
  });
